...
def ship_delivery_group(request, order_pk, group_pk):
    ...
    if form.is_valid():
        with transaction.atomic():
            form.save()
            for _ord in order.get_items():
                pv = ProductVariant.objects.get(product=_ord.product)
                for _stk in pv.stock.all():
                    if _stk.quantity > 0:
                        if _stk.quantity > _ord.quantity:
                            _stk.quantity -= _ord.quantity
                            _stk.save()
